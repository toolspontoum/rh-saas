...
export async function POST(
  ...
  const { runTenantNoticeReadPost } = await import("@vv/api/run-tenant-writes");
  const auth = request.headers.get("authorization");
  const companyHeader = request.headers.get("x-tenant-company-id");
  const out = await runTenantNoticeReadPost(auth, tenantId, noticeId, companyHeader ?? undefined);
