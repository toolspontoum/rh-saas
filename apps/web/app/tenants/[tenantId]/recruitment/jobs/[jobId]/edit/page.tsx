...
function parseExperienceList(roleRaw: string | null, monthsRaw: number | null): ExperienceCriterion[] {
  const roleText = roleRaw?.trim() ?? "";
  if (!roleText) return [];

  const parts = roleText
    .split(";")
    .map((part) => part.trim())
    .filter(Boolean);
    ...
    return [
      {
        ...
        role: roleText,
        months: monthsRaw && monthsRaw > 0 ? Math.round(monthsRaw) : null
      }
    ];
  }

  return parts.map((part, index) => {
    const m = part.match(/^(.*)\((\d+)\s*meses\)$/i);
    const role = (m ? m[1] : part).trim();
    const months = m ? Number.parseInt(m[2], 10) : null;
    return {
      id: typeof crypto !== "undefined" && "randomUUID" in crypto ? crypto.randomUUID() : `exp-${Date.now()}-${index}`,
      role,
      months: Number.isFinite(months) && months && months > 0 ? months : null
    };
  });
}
...
function formatIsoToDateBr(value: string | null): string {
  if (!value) return "";
...
export default function RecruitmentEditJobPage() {
  ...
  const [status, setStatus] = useState<Job["status"]>("published");
  ...
  useEffect(() => {
    apiFetch<Job>(`/v1/tenants/${tenantId}/jobs/${jobId}`)
      .then((job) => {
        ...
        setCity(job.city ?? "");
        setState(job.state && job.state.length <= 2 ? job.state : "");
        setSalary(job.salary == null ? "" : formatSalaryCurrency(String(Math.round(job.salary * 100))));
        setExpiresAt(formatIsoToDateBr(job.expiresAt));
        setSkills(job.skills ?? []);
        setQuestions(
          (job.screeningQuestions ?? [])
            .filter((q: { type: string }) => q.type !== "document_upload")
            .map((q) => ({
              ...q,
              expectedAnswer: q.expectedAnswer ?? "yes",
              isEliminatory: q.type === "text" ? false : q.isEliminatory,
              isRequired: q.isEliminatory ? true : q.isRequired
            })) as JobQuestion[]
        );
        setDocumentRequirements(
          (job.documentRequirements ?? []).map((r) => ({
            id: r.id,
            docTab: r.docTab as JobDocumentRequirementValue["docTab"],
            docType: r.docType,
            label: r.label
          }))
        );
        const ai = job.aiScreeningCriteria;
        if (ai) {
          setAiKeywords(Array.from(new Set((ai.keywords ?? []).map(normalizeTagToken).filter(Boolean))));
          setAiFormation(
            Array.from(
              new Set(
                (ai.formation ?? "")
          ...
          setAiCertificates(Array.from(new Set((ai.certificates ?? []).map(normalizeTagToken).filter(Boolean))));
          setAiExperienceList(parseExperienceList(ai.experienceRole ?? null, ai.experienceMonths ?? null));
        } else {
          setAiKeywords([]);
          setAiFormation([]);
          setAiCertificates([]);
      ...
      <div className="card">
        <form className="stack" onSubmit={onSubmit}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "33fr 34fr 33fr", gap: 12, alignItems: "start" }}>
            <input value={department} onChange={(e) => setDepartment(e.target.value)} placeholder="Área (Departamento)" />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "20fr 45fr 35fr", gap: 12, alignItems: "start" }}>
            ...
            <input list="city-options-edit" value={city} onChange={(e) => setCity(e.target.value)} placeholder="Cidade" disabled={!state} />
          ...
          <div className="card stack" style={{ padding: 12 }}>
            ...
            <label>
              Habilidades da vaga
              <SkillTagsInput value={skills} onChange={setSkills} placeholder="Digite e pressione Enter (ex.: excel, atendimento)" allowCreate />
            </label>
            <label>
              Palavras-chave para triagem
              <SkillTagsInput value={aiKeywords} onChange={setAiKeywords} placeholder="Digite e pressione Enter" allowCreate />
            </label>
            <label>
              Formação esperada
              <SkillTagsInput value={aiFormation} onChange={setAiFormation} placeholder="Digite e pressione Enter" allowCreate />
            </label>
            <label>
              Certificados desejados
              <SkillTagsInput value={aiCertificates} onChange={setAiCertificates} placeholder="Digite e pressione Enter" allowCreate />
            </label>
            <div
              className="stack"
              style={{
          ...
          <div className="card stack" style={{ padding: 12 }}>
            <div className="section-header">
              <h3>Perguntas</h3>
              <button type="button" className="secondary" onClick={() => setQuestions((c) => [...c, createQuestion()])}>
                + Adicionar pergunta
              </button>
            </div>
