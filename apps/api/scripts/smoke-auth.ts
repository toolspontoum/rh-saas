...
async function callApi(path: string, token: string, method: "GET" | "POST" = "GET") {
...
async function main() {
  ...
  const meTenants = await callApi("/v1/me/tenants", access_token);
  const tenantContext = await callApi(`/v1/tenants/${tenantId}/context`, access_token);
  const validateFeature = await callApi(
    `/v1/tenants/${tenantId}/features/mod_recruitment/validate`,
    access_token,
    "POST"
  );
